...
dim_state=2; dim_mediator = 2
std_M = 1; std_S = 1
simulator = Simulator(model_type='Gaussian_semi_s2m2', dim_state=dim_state, dim_mediator = dim_mediator, std_M = std_M, std_S = std_S)
...
nrep = 25
Error = dict()
for num_time in np.arange(1, 6) * 300:
    Error[num_time] = dict()
    for num_trajectory in np.arange(1, 2) * 100:
        Error[num_time][num_trajectory]= MC_toy_robust(nrep,num_trajectory, num_time, 50, 500)
        with open("s2m2_estimate_additive_identity_L7_stdM_"+str(std_M)+"_stdS_"+str(std_S)+'_Tbaselines.txt',"wb") as fp:
