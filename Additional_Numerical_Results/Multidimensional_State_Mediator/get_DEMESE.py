...
def mc_DE_ME_SE(num_trajectory=500, num_time=500, seed=1):
    ...
    target_iid_dataset = simulator.trajectory2iid(
        trajectory=target_trajectories)
    ...
    control_trajectories = simulator.sample_target_policy_trajectory(
        ...
        policy="control",
        return_trajectory=True)
    control_iid_dataset = simulator.trajectory2iid(
        trajectory=control_trajectories)
...
def est_eta1(state, a0, unique_action):
    ...
    for a in unique_action:
        ...
        for rep in range(expectation_MCMC_iter):
            ...
            reward_i_a = simulator.sam2reward_model(
                state,
                action=a,
                mediator=m_i_a,
                random=False,
                matrix_based=True).flatten()
...
def est_eta2(state, a0, unique_action):
    ...
    for a in unique_action:
        ...
        for rep in range(expectation_MCMC_iter):
            m_i_a = simulator.sa2mediator_model(state, a, random=True)
            reward_i_a0 = simulator.sam2reward_model(
                state,
                action=a0,
                mediator=m_i_a,
                random=False,
...
def est_eta3(state, a0, unique_action):
    ...
    for rep in range(expectation_MCMC_iter):
        ...
        reward_i_a0 = simulator.sam2reward_model(state,
                                                 action=a0,
                                                 mediator=m_i_a0,
                                                 random=False,
                                                 matrix_based=True).flatten()
...
def estimate_DE_ME_SE(mc_s0_time, num_trajectory, num_time):
    ...
    num_trajectory_list = (np.ones(mc_s0_time, dtype='int64') *
    ...
    with Pool(CORES) as p:
        DE_ME_SE = list(
            tqdm(p.imap(mc_DE_ME_SE_star, param_list), total=mc_s0_time))
    return DE_ME_SE


def main(save_file):
    DE_ME_SE = dict()
    # for num_time in [100, 200, 400, 800, 1600, 3200, 6400]:
    for num_time in [800, 1600, 3200, 6400]:
        DE_ME_SE[num_time] = dict()
        for num_trajectory in [100]:
            DE_ME_SE[num_time][num_trajectory] = np.mean(
                estimate_DE_ME_SE(200, num_trajectory, num_time), 0)
            with open(save_file, "wb") as fp:
                pickle.dump(DE_ME_SE, fp)
    return DE_ME_SE


DE_ME_SE = main("s2m2_true_iDE_IME_DDE_DME_direct_est_stdM_" + str(std_M) +
                "_stdS_" + str(std_S) + '.txt')
