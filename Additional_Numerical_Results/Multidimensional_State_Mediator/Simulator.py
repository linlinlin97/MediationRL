...
# define the environment of the toy --Gaussian
    ...
    def Gaussian_sam2reward_model(self,
                                  state,
                                  action,
                                  mediator,
                                  random,
                                  matrix_based=False):  #, mediator
        if matrix_based:
            #print("matrix")
            state1 = np.copy(state).flatten()
            action1 = np.copy(action).flatten()
            mediator1 = np.copy(mediator).flatten()
            rmean = .7 * state1 + .6 * mediator1 + .9 * (action1 - .5) + (
                .4 * np.sqrt(abs(mediator1))) * (action1 - .5)
            rmean *= 5
            if random:
                raise ValueError('No random for matrix input')
            else:
            ...
            rmean = .7 * state + .6 * mediator + .9 * (action - .5) + (
                .4 * np.sqrt(abs(mediator))) * (action - .5)
            rmean *= 5
            if random:
...
# define the environment of the semi-synthetic --Gaussian
    ...
    def semi_sa2mediator_model(self, state, action, random):
        ...
        if self.dim_mediator == 2:
            pm1 = np.sqrt(abs(state.reshape((-1, 1)))) + (action.reshape(
                (-1, 1)) - .5)
            pm2 = -.5 * state.reshape((-1, 1)) + .5 * (action.reshape(
                (-1, 1)) - .5) * np.sqrt(abs(state.reshape((-1, 1))))
            pm = np.hstack([pm1, pm2])

        pm = pm.reshape((N, self.dim_mediator))
        if random:
            #pm += np.random.normal(loc=0, scale=1, size=1)
            pm += self.std_M * np.random.normal(
                loc=0, scale=1, size=(N, self.dim_mediator))
        else:
            pm = pm
        return pm

    def semi_sam2reward_model(self,
                              state,
                              action,
                              mediator,
                              random,
        ...
        if matrix_based:
            ...
            if self.dim_mediator == 2:
                ...
                rmean += .5 * np.sqrt(
                    abs(mediator1[:, 0]).reshape((-1, 1)) +
                    abs(mediator1[:, 1]).reshape((-1, 1))) * (action1 - .5)
                rmean += .5 * (action1 - .5)
                rmean *= 1.5
            ...
            if self.dim_mediator == 2:
                ...
                rmean += 1 * mediator[0]
                rmean += 1 * mediator[1]
                rmean += .5 * np.sqrt(abs(mediator[0]) +
                                      abs(mediator[1])) * (action - .5)
                rmean += .5 * (action - .5)
                rmean *= 1.5
            if random:
                reward = rmean + self.std_S * np.random.normal(
...
# define the environment of the semi-synthetic --Gaussian
    ...
    def semis3m2_init_state(self):
        init_state = np.random.normal(loc=0, scale=1, size=self.dim_state)
        return init_state

    def semis3m2_s2action_model(self, state, random):
        pa = .5
        if random:
            pa = np.random.binomial(n=1, p=pa, size=1)
        else:
    ...
    def semis3m2_sa2mediator_model(self, state, action, random):
        state = state.reshape((-1, self.dim_state))
        N = state.shape[0]
        if self.dim_mediator == 2:
            pm1 = np.mean(np.sqrt(
                np.abs(state.reshape((-1, self.dim_state)))),
                        axis=1).reshape(-1, 1) + (action.reshape(
                            (-1, 1)) - .5)
            pm2 = -.5 * np.mean(state.reshape(
                (-1, self.dim_state)), axis=1).reshape(
                    (-1, 1)) + .5 * (action.reshape(
                        (-1, 1)) - .5) * np.mean(np.sqrt(
                            abs(state.reshape((-1, self.dim_state)))),
                                                 axis=1).reshape(-1, 1)
            pm = np.hstack([pm1, pm2])
    ...
    def semis3m2_sam2reward_model(self,
                                  state,
                                  action,
                                  mediator,
                                  random,
        ...
        if matrix_based:
            ...
            if self.dim_mediator == 2:
                ...
                action1 = action.reshape((-1, 1))
                rmean = 0.75 * (
                    np.mean(state1, axis=1).reshape(-1, 1) +
                    np.mean(np.sqrt(np.abs(state1)), axis=1).reshape(-1, 1) +
                    (1 + np.sqrt(
                        np.abs(mediator1[:, [0]]) + np.abs(mediator1[:, [1]])))
                    * (action1 - 0.5))
                rmean += 1.5 * (mediator1[:, [0]] + mediator1[:, [1]])
            ...
            if self.dim_mediator == 2:
                rmean = 0.75 * (
                    np.mean(state) + np.mean(np.sqrt(np.abs(state))) +
                    (1 + np.sqrt(np.abs(mediator[0]) + np.abs(mediator[1]))) *
                    (action - 0.5))
                rmean += 1.5 * (mediator[0] + mediator[1])
            if random:
                reward = rmean + self.std_S * np.random.normal(
                    loc=0, scale=1, size=1)
            else:
    ...
    def semis3m2_sam2nextstate_model(self, state, action, mediator, reward):
        random = True
        next_state_mean = np.zeros_like(state)

        next_state_mean[0] = 0.75 * (
            state[0] + np.sqrt(np.abs(state[0])) +
            (1 + np.sqrt(np.abs(mediator[0]) + np.abs(mediator[1]))) *
            (action - 0.5)) + 1.5 * (mediator[0] + mediator[1])
        next_state_mean[1] = 0.75 * (
            state[1] + np.sqrt(np.abs(state[1])) +
            (1 + np.sqrt(np.abs(mediator[0]) + np.abs(mediator[1]))) *
            (action - 0.5)) + 1.5 * (mediator[0] + mediator[1])
        next_state_mean[2] = 0.75 * (
            state[2] + np.sqrt(np.abs(state[2])) +
            (1 + np.sqrt(np.abs(mediator[0]) + np.abs(mediator[1]))) *
            (action - 0.5)) + 1.5 * (mediator[0] + mediator[1])

        if random:
            next_state = next_state_mean + self.std_S * np.random.normal(
                loc=0, scale=1, size=self.dim_state)
        else:
...
# define the environment of the semi-synthetic --Gaussian
    ...
    def semis2m2_init_state(self):
        init_state = np.random.normal(loc=0, scale=1, size=self.dim_state)
        return init_state

    def semis2m2_s2action_model(self, state, random):
        pa = .5
        if random:
            pa = np.random.binomial(n=1, p=pa, size=1)
        else:
    ...
    def semis2m2_sa2mediator_model(self, state, action, random):
        state = state.reshape((-1, self.dim_state))
        N = state.shape[0]
        if self.dim_mediator == 2:
            pm1 = np.mean(np.sqrt(
    ...
    def semis2m2_sam2reward_model(self,
                                  state,
                                  action,
                                  mediator,
                                  random,
    ...
    def semis2m2_sam2nextstate_model(self, state, action, mediator, reward):
        random = True
        next_state_mean = np.zeros_like(state)
...
# Initiate the simulator
    ...
    def __init__(self,
        ...
        elif model_type == "Gaussian_semi_s3m2":
            self.model_type = "Gaussian_semi_s3m2"
            self.init_state_model = self.semis3m2_init_state
            self.s2action_model = self.semis3m2_s2action_model
            self.sa2mediator_model = self.semis3m2_sa2mediator_model
            self.sam2reward_model = self.semis3m2_sam2reward_model
            self.sam2nextstate_model = self.semis3m2_sam2nextstate_model
        elif model_type == "Gaussian_semi_s2m2":
            self.model_type = "Gaussian_semi_s2m2"
            self.init_state_model = self.semis2m2_init_state
            self.s2action_model = self.semis2m2_s2action_model
            self.sa2mediator_model = self.semis2m2_sa2mediator_model
            self.sam2reward_model = self.semis2m2_sam2reward_model
            self.sam2nextstate_model = self.semis2m2_sam2nextstate_model
...
# utility functions used to sample random action/mediator...
    ...
    def sample_sam2reward(self,
                          state,
                          action,
                          mediator,
                          random=True,
                          matrix_based=False):
        random_reward = self.sam2reward_model(state,
                                              action,
                                              mediator,
                                              random=random,
    ...
    def sample_sam2nextstate(self, state, action, mediator, reward):
        random_next_state = self.sam2nextstate_model(state, action, mediator,
                                                     reward)
...
# Sample trajectories
    ...
    def sample_one_trajectory(self, num_time, burn_in):
        ...
        for i in range(num_time):
            random_action[i] = self.sample_s2action(random_state[i])
            random_mediator[i] = self.sample_sa2mediator(
                random_state[i], random_action[i])
            random_reward[i] = self.sample_sam2reward(random_state[i],
                                                      random_action[i],
                                                      random_mediator[i])
        ...
        random_trajectory = [
            random_state, random_action, random_mediator, random_reward,
            time_idx
        ]
    ...
    def sample_one_target_policy_trajectory(self,
                                            num_time,
                                            target_policy,
                                            control_policy,
                                            burn_in,
                                            sample_policy='control'):
        '''
        Output: A list containing 4 elements: state, action, mediator, reward
        '''
        if burn_in:
        ...
        for i in range(num_time):
            if sample_policy == 'control':
                random_action[i] = control_policy(state=random_state[i],
                                                  dim_state=self.dim_state)
                action_t = random_action[i]
            elif sample_policy == 'target':
                random_action[i] = target_policy(state=random_state[i],
                                                 dim_state=self.dim_state)
                action_t = random_action[i]
            else:
                random_action[i] = control_policy(state=random_state[i],
                                                  dim_state=self.dim_state)
                action_t = target_policy(state=random_state[i],
                                         dim_state=self.dim_state)
    ...
    def sample_target_policy_trajectory(self,
                                        num_trajectory,
                                        num_time,
                                        seed,
                                        policy="target",
    ...
    def trajectory2iid(self, trajectory=None):
        ...
        for i in range(num_trajectory):
            ...
            if i == 0:
                ...
                iid_dataset[0] = np.vstack([iid_dataset[0],
                                            iid_data[0]])  #state 3-d
                iid_dataset[4] = np.vstack([iid_dataset[4],
                                            iid_data[4]])  #nextstate 3-d
                iid_dataset[1] = np.append(iid_dataset[1],
                                           iid_data[1])  #action 1-d
                iid_dataset[2] = np.vstack([iid_dataset[2],
                                            iid_data[2]])  #mediator 2-d
                iid_dataset[3] = np.append(iid_dataset[3],
                                           iid_data[3])  #reward 1-d
                iid_dataset[5] = np.append(iid_dataset[5],
                                           iid_data[5])  #time 1-d
        ...
        if trajectory is None:
            self.iid_dataset = {
                ...
                'mediator': iid_dataset[2],
                'reward': iid_dataset[3],
