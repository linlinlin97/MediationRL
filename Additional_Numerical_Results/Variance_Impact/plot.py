...
def plot_variance(result, x='NT', effect = 'IDE'):
    fig, ((ax1, ax2, ax3) ,(ax5, ax6, ax7)) = plt.subplots(nrows=2, ncols=3, sharex=True, sharey=False, figsize = (10,5))
    
    COLORS = sns.color_palette("colorblind")
    palette = {'MR' : COLORS[0],'DM' : COLORS[1], 'MIS1': COLORS[3], 'MIS2': COLORS[4],'Baseline_IPW':COLORS[5],'Baseline_MR':COLORS[6],'Baseline_DM':COLORS[2]}

    ax1 = sns.lineplot(data=result[result.std_sm == 1], x=x, y=effect+"_error", hue="estimand",
                       style="estimand", ci = 95, err_style="bars", ax = ax1,
                       n_boot = 20, palette = palette,linewidth = 2.0,
                       markers = True)
    ax1.set_title(r'$\sigma_s = \sigma_m = \sigma_r = 1$', fontsize = 18)
    ax1.tick_params(axis='y', labelsize=11)
    ax1.axes.set_ylabel("logbias", fontsize = 18)
    
    ax2 = sns.lineplot(data=result[result.std_sm == 2], x=x, y=effect+"_error", hue="estimand",
                       style="estimand", ci = 95, err_style="bars", ax = ax2,
                       n_boot = 20, palette = palette,linewidth = 2.0,
                       markers = True)
    ax2.set_title(r'$\sigma_s = \sigma_m = \sigma_r = 2$', fontsize = 18)
    ax2.tick_params(axis='y', labelsize=11)
    ax2.set(ylabel=None)
    
    ax3 = sns.lineplot(data=result[result.std_sm == 3], x=x, y=effect+"_error", hue="estimand",
                       style="estimand", ci = 95, err_style="bars", ax = ax3,
                       n_boot = 20, palette = palette,linewidth = 2.0,
                       markers = True)
    ax3.set_title(r'$\sigma_s = \sigma_m = \sigma_r = 3$', fontsize = 18)
    ax3.tick_params(axis='y', labelsize=11)
    ax3.set(ylabel=None)
    

    ax5 = sns.lineplot(data=result[result.std_sm == 1], x=x, y=effect+"_MSE", hue="estimand",
                       style="estimand", ci = 95, err_style="bars",
                       ax = ax5, n_boot = 20, palette = palette,
                       linewidth = 2.0,markers = True)
    ax5.axes.set_ylabel("logMSE", fontsize = 18)
    ...
    ax6= sns.lineplot(data=result[result.std_sm == 2], x=x, y=effect+"_MSE", hue="estimand",
                       style="estimand", ci = 95, err_style="bars",
                       ax = ax6, n_boot = 20, palette = palette,
                       linewidth = 2.0,markers = True)
    ax6.set(ylabel=None)
    ...
    ax7= sns.lineplot(data=result[result.std_sm == 3], x=x, y=effect+"_MSE", hue="estimand",
                       style="estimand", ci = 95, err_style="bars",
                       ax = ax7, n_boot = 20, palette = palette,
                       linewidth = 2.0,markers = True)
    ax7.set(ylabel=None)
    ...
    plt.tight_layout()
    plt.show()
    fig.savefig('Synthetic_Final_Variance_'+effect+'_'+str(x), bbox_inches='tight')