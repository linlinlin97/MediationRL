...
nrep = 50
Error = dict()
for num_time in np.arange(1, 2) * 50:
    Error[num_time] = dict()
    for num_trajectory in np.arange(1, 6) * 300:
        Error[num_time][num_trajectory]= MC_toy_robust(nrep,num_trajectory, num_time, 20,20,20, 15, 500)
        with open("estimate_additive_identity_L7_stdM_"+str(std_M)+"_stdS_"+str(std_S)+'.txt',"wb") as fp:
