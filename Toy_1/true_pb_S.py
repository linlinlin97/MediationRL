...
def mc_eta_target(num_trajectory, num_time, seed, policy = "control"):
    if policy == "behavior":
        trajectory_b = simulator.sample_trajectory( num_trajectory, num_time, seed, burn_in=False, return_trajectory=True)
        est_ratio_target = np.mean(np.vstack([_trajectory[0].flatten() for _trajectory in trajectory_b]),0)
        out = [num_trajectory, num_time, est_ratio_target]
    else: 
...
def estimate_eta(mc_s0_time, num_trajectory, num_time):
    ...
    policy_list = ["behavior"]*mc_s0_time
...
for num_trajectory in [500]:
    ...
    for num_time in [2000]:
        ...
        with open("true_pb_S.txt","wb") as fp:
...
#    with open("true_pb_S_simple.txt","wb") as fp:
#        pickle.dump(ETA_list, fp)
