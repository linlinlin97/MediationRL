...
class evaluator:
    def __init__(self, dataset,
        ...
        self.eta_pi, self.eta_pia0, self.eta_pia0star, self.eta_a0 = self.etalearner.get_eta_prediction()
    ...
    def estimate_DE_ME_SE(self):
        ...
        intercept_DE = self.eta_pi-self.eta_pia0
        intercept_ME = self.eta_pia0-self.eta_pia0star
        intercept_SE = self.eta_pia0star-self.eta_a0
        ...
        #SE
        self.ind_est[2] = (self.eta_pia0star-self.eta_a0) +termI3 - termI4 # 
    ...
    def compute_termI2(self, data_num, state, action, reward, next_state, mediator):
        ...
        #termI2 = pAS_ratio_a0b * (reward - Er_SAM)
    ...
    def compute_termI3(self, data_num, state, action, reward, next_state, mediator):
        ...
        ## third
        #pM_Sa0 = self.pmlearner.get_pm_prediction(state, action, self.a0)
        
        termI3 = pAS_ratio_a0b * (reward - Er_SA)
        #termI3 = pM_S / pM_SA * pAS_ratio_a0b * (reward - Er_SAM)
        #termI3 = pM_Sa0 / pM_SA * pAS_ratio_a0b * (reward - Er_SAM)
    ...
    def compute_termI4(self, data_num, state, action, reward, next_state):
        ...
        Q4_Sa0_prime = np.copy(self.Q4_a0_Sprime)
        
        Q4_SA = np.copy(self.Q4_SA)
        
        termI4 = w_a0 * pAS_ratio_a0b * (reward + Q4_Sa0_prime - Q4_SA - self.eta_a0)
    ...
    def compute_Q(self, data_num, state, action, next_state):
        ...
        self.Q4_a0_Sprime = np.zeros(data_num, dtype=float)
        for a in self.unique_action:
        ...
        for m in self.unique_mediator:
            ...
            Q4_SAm = self.qlearner4.get_q_prediction(state, action, m)
            self.Q1_SA += Q1_SAm * pm_AS
            self.Q2_SA += Q2_SAm * pm_AS
            self.Q3_SA += Q3_SAm * pm_AS
            self.Q4_SA += Q4_SAm * pm_AS