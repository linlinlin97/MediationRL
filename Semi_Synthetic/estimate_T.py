...
def toy_robust(num_trajectory, num_time, expectation_MCMC_iter = 100, expectation_MCMC_iter_Q3 = 100, expectation_MCMC_iter_Q_diff = 100, 
    ...
    est_obj1 = evaluator(sim_iid_dataset, num_trajectory, num_time,
                         ...
                         dim_state = 1, dim_mediator = 2,
                         Q_settings = {'scaler': 'Identity','product_tensor': False, 'beta': 2/7, 'include_intercept': False,
                                       'expectation_MCMC_iter_Q3': expectation_MCMC_iter_Q3, 
                                       'expectation_MCMC_iter_Q_diff':expectation_MCMC_iter_Q_diff,
                                       'penalty': 10**(-4), 'd': 3, 'min_L': 7, "t_dependent_Q": False},
    ...
    return [IDE_MR, IME_MR, DDE_MR, DME_MR,
            IDE_Direct, IME_Direct, DDE_Direct, DME_Direct,
            IDE_WIS1, IME_WIS1, DDE_WIS1, DME_WIS1,
            IDE_WIS2, IME_WIS2, DDE_WIS2, DME_WIS2,
            IDE_baseline, IME_baseline, DDE_baseline, DME_baseline,
            num_trajectory, num_time, seed, est_obj1.eta_pie, est_obj1.eta_a0]
...
nrep = 100
Error = dict()
for num_time in np.arange(1, 16) * 100:
    Error[num_time] = dict()
    for num_trajectory in np.arange(1, 2) * 50:
        Error[num_time][num_trajectory]= MC_toy_robust(nrep,num_trajectory, num_time, 50,50,50, 15, 500)
        with open("estimate_additive_identity_L7_stdM_"+str(std_M)+"_stdS_"+str(std_S)+'_T.txt',"wb") as fp:
