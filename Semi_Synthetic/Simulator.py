...
# define the environment of the toy --Gaussian
    ...
    def Gaussian_sa2mediator_model(self, state, action, random):
        N = state.shape[0]
        pm =  .3 * state + .8 * (action - .5)
        pm = pm.reshape((N,))
        if random:
            #pm += np.random.normal(loc=0, scale=1, size=1)
            pm += 2 * np.random.normal(loc=0, scale=1, size=(N,))
        else:
            pm = pm
        return pm
    ...
    def Gaussian_sam2reward_model(self, state, action,mediator, random, matrix_based = False):#, mediator
        if matrix_based:
            ...
            action1 = np.copy(action).flatten()
            mediator1 = np.copy(mediator).flatten()
            rmean = .5 * state1 +  .9 * mediator1 + (.6 * np.sqrt(abs(mediator1)) ) * (action1 - .5)
            rmean *= 5
            if random:
                print("wrong")
            else:
            ...
            rmean = .5 * state +  .9 * mediator + (.6 * np.sqrt(abs(mediator))) * (action - .5)
            rmean *= 5
            if random:
