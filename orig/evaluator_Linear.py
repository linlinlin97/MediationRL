...
class evaluator:
    ...
    def _I_terms(self, data_num):
        self.Er_Sa0, self.Er_SA = self._Er_Sa0_SA(data_num, self.state, self.action)
    ...
    def compute_termI5(self, data_num, state, action, reward, next_state):
        reward = np.copy(reward).flatten()
        termI5 = self.w_a0 * self.I_A / self.pieb_A * (reward + self.Q5_diff - self.eta_a0)
        
        return termI5
    
    def _Er_Sa0_SA(self, data_num, state, action):
